    ...
    for (let x = CANVAS_MIN_X; x < CANVAS_MAX_X; x++) {
        ...
        for (let y = CANVAS_MIN_Y; y < CANVAS_MAX_Y; y++) {
            if (cachedCells[y - CANVAS_MIN_Y] !== null) {
                ...
                if (cell !== undefined) {
                    const p = PixelToNetwork({ color_id: cell.color_id, username: cell.username, set_time: parseInt(cell.set_time) });
                    board[x - CANVAS_MIN_X][y - CANVAS_MIN_Y] = p;
                } //
                else {
                    const p = PixelToNetwork({ color_id: 1, username: 'null', set_time: 0 });
                    board[x - CANVAS_MIN_X][y - CANVAS_MIN_Y] = p;
                }
